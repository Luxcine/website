        ...
        <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-[#E8E4DC]/6 border border-[#E8E4DC]/6 mb-10">
          {[
            ...
            <div key={s.label} className="px-8 py-6">
              ...
              <div className="text-[9px] tracking-[0.2em] uppercase text-[#E8E4DC]/25">{s.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 divide-x divide-[#E8E4DC]/6 border border-[#E8E4DC]/6 mb-10">
          {data?.dayStats.map(d => (
            <div key={d.date} className="px-5 py-5">
              <div className="text-[9px] tracking-[0.15em] uppercase text-[#E8E4DC]/25 mb-2">
                {DAY_LABELS[d.date]}
              </div>
              <div className="font-['Cormorant_Garamond'] text-2xl font-light text-[#E8E4DC]/60 mb-0.5">
                {d.totalSeats}
              </div>
              <div className="text-[9px] text-[#E8E4DC]/20">guests booked</div>
            </div>
          ))}
        </div>
          ...
          <div className="flex gap-6 border-b border-[#E8E4DC]/6">
            {(['bookings', 'calendar'] as const).map(v => (
              <button key={v} onClick={() => setView(v)}
                className={`text-[9px] tracking-[0.25em] uppercase pb-3 transition-colors duration-300 ${
                  view === v ? 'text-[#9C8660] border-b border-[#9C8660]' : 'text-[#E8E4DC]/25 hover:text-[#E8E4DC]/50'
                }`}>
              ...
              <>
                ...
                <div className="grid grid-cols-12 gap-4 px-6 py-3 border-b border-[#E8E4DC]/6 bg-[#0F0F0F]">
                  {['Ref', 'Name', 'Date / Time', 'Guests', 'Company', 'Role', 'Location', ''].map((h, i) => (
                    <div key={i} className={`text-[8px] tracking-[0.25em] uppercase text-[#E8E4DC]/25 ${i === 0 ? 'col-span-1' : i === 1 ? 'col-span-2' : i === 2 ? 'col-span-2' : i === 4 ? 'col-span-2' : i === 6 ? 'col-span-2' : 'col-span-1'}`}>
                      {h}
                    </div>
                  ))}
                </div>
                ...
                {filtered.map((b, i) => (
                  <div key={b.ref}
                    className={`grid grid-cols-12 gap-4 px-6 py-5 border-b border-[#E8E4DC]/4 hover:bg-[#0F0F0F] transition-colors duration-300 ${i % 2 === 0 ? '' : 'bg-[#080808]'}`}>
                    <div className="col-span-1">
                      <span className="text-[10px] font-mono text-[#9C8660]/70">{b.ref}</span>
                    </div>
                    <div className="col-span-2">
                      <div className="text-[12px] text-[#E8E4DC]/70 font-light">{b.name}</div>
                      <div className="text-[10px] text-[#E8E4DC]/30 mt-0.5">{b.email}</div>
                      {b.phone && <div className="text-[10px] text-[#E8E4DC]/20 mt-0.5">{b.phone}</div>}
                    </div>
                    <div className="col-span-2">
                      <div className="text-[12px] text-[#E8E4DC]/60">{DAY_LABELS[b.date] ?? b.date}</div>
                      <div className="text-[10px] text-[#E8E4DC]/30 mt-0.5">{b.slot}</div>
                    </div>
                    <div className="col-span-1">
                      <span className="text-[12px] text-[#E8E4DC]/50">{b.guests}</span>
                    </div>
                    <div className="col-span-2">
                      <span className="text-[11px] text-[#E8E4DC]/40 font-light">{b.company ?? '—'}</span>
                    </div>
                    <div className="col-span-1">
                      <span className="text-[10px] text-[#E8E4DC]/30">{b.role ?? '—'}</span>
                    </div>
                    <div className="col-span-2">
                    ...
                    <div className="col-span-1 flex flex-col items-end gap-2">
                      ...
                      <button
                        ...
                        className="text-[9px] tracking-[0.15em] uppercase text-red-400/30 hover:text-red-400/60 transition-colors duration-300 disabled:opacity-30">
                        {cancelling === b.ref ? '...' : 'Cancel'}
                      </button>
                    </div>
                  </div>
                ))}
